...
CHANGELOG 与 Release 说明由脚本从提交自动归一化生成，请让每条版本要点“条目具体、描述简短”：

- 一条要点只写一件事，突出「做了什么 / 结果」，不写实现过程、背景与冗余修饰。
- 脚本只按全角句末标点（`。！？`）把「一行多句」自动拆成多条**完整**要点，**不会截断丢字**；所以「描述简短」要靠提交时一行只写一件事来保证（建议单条 ≤ 20 字），需要并列的多个要点请各自写成一行。
- 需要展开的细节写进提交正文的多行要点或 PR 描述，不要堆在单条标题里。
