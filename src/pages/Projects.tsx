...
export default function Projects({ embedded = false, onChanged }: { embedded?: boolean; onChanged?: () => void | Promise<void> }) {
  ...
  const load = async () => { try { const [projectList, actionList] = await Promise.all([projectsApi.list(), actionsApi.list()]); setProjects(projectList); setActions(actionList); setError(""); setExpanded((current) => current.length === 0 && projectList.length > 0 ? [String(projectList[0].id)] : current); } catch (cause) { setError(cleanError(cause)); } };
  useEffect(() => { void load(); }, []);
  ...
  const deleteProject = async (item: Project) => { try { await projectsApi.delete(item.id); await load(); message.success("项目已删除"); } catch (cause) { setError(cleanError(cause)); } };
  const completeProject = async (item: Project) => { try { await projectsApi.complete(item.id); await load(); await onChanged?.(); message.success("项目已完成"); } catch (cause) { setError(cleanError(cause)); } };
  const deleteAction = async (item: Action) => { try { await actionsApi.delete(item.id); await load(); message.success("行动已删除"); } catch (cause) { setError(cleanError(cause)); } };
  ...
  return <div className={embedded ? "projects-embedded" : "page"}>{!embedded && <header className="page-header"><div><Typography.Title level={2} className="page-title">项目</Typography.Title><Typography.Paragraph className="page-subtitle">从事件转化而来的可执行结果，拆成一条条行动。</Typography.Paragraph></div></header>}
    ...
    <AbandonModal project={abandoning} onClose={() => setAbandoning(null)} onSaved={async (reason) => { if (abandoning) { try { await projectsApi.abandon(abandoning.id, reason); setAbandoning(null); await load(); message.success("项目已放弃"); } catch (cause) { setError(cleanError(cause)); } } }} />
  </div>;
}
