    ...
    return (
      ...
      >
        <List.Item.Meta
          ...
          title={<span className={`project-action-title ${action.status === 1 ? "completed-title" : ""}`}><span className="project-action-number" aria-label={`第${actionIndex + 1}个行动`}>{actionNumber}</span>{action.title}</span>}
...
function ProjectActionModal({ project, action, defaultStartDate, onClose, onSaved }: { project: Project | null; action: Action | null | undefined; defaultStartDate?: string; onClose: () => void; onSaved: () => Promise<void> }) {
  const [form] = Form.useForm();
  useEffect(() => { if (project && action !== undefined) form.setFieldsValue(action ? { title: action.title, description: action.description, estimated_hours: action.estimated_hours, start_date: action.start_date ? dayjs(action.start_date) : undefined, deadline: action.deadline ? dayjs(action.deadline) : undefined, is_frog: action.is_frog === 1 } : { estimated_hours: undefined, start_date: defaultStartDate ? dayjs(defaultStartDate) : undefined, is_frog: false }); }, [project, action, form]);
  if (!project || action === undefined) return null;
  const submit = async (values: Record<string, unknown>) => { const base = { title: values.title as string, description: values.description as string || undefined, estimated_hours: values.estimated_hours ? Number(values.estimated_hours) : 0, start_date: formatDate(values.start_date as Dayjs | undefined), deadline: formatDate(values.deadline as Dayjs | undefined), is_frog: values.is_frog ? 1 : 0, importance: project.importance, urgency: project.urgency }; try { if (action) await actionsApi.update({ ...base, id: action.id } as UpdateAction); else { await actionsApi.create({ ...base, project_id: project.id }); track("创建项目行动", { estimated_hours: base.estimated_hours, is_frog: base.is_frog === 1 }); } await onSaved(); message.success(action ? "行动已更新" : "行动已添加"); } catch (cause) { message.error(userFacingError(cause)); } };
