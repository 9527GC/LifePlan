...
function EventActionPicker({ actions, slots, saving, onAssign }: { actions: Action[]; slots: DailyScheduleSlot[]; saving: boolean; onAssign: (id: number) => void }) {
  ...
  const groups = sortByPriority(actions.filter((action) => action.status !== 1).reduce<Array<{ key: string; eventTitle: string; projectTitle?: string; actions: Action[]; priority: number }>>((result, action) => {
    ...
    else result.push({ key, eventTitle: action.event_title || "所属事件", projectTitle: action.project_title, actions: [action], priority: action.priority });
    return result;
