...
import { Check, Edit3, GripVertical, ListChecks, LoaderCircle, Plus, RotateCcw, Trash2 } from "lucide-react";
import { actionsApi, eventsApi } from "@/lib/api";
import type { Action, Event, ProcessActionStep, ProcessEvent, UpdateAction } from "@/types";
...
const formatDuration = (value: number) => value === 0 ? "未设置预计耗时" : value === 0.5 ? "30分钟" : `${value}小时`;
...
const sortEventActions = (actions: Action[]) => [...actions].sort((left, right) => {
  ...
  return leftOrder !== rightOrder ? leftOrder - rightOrder : compareActionDate(left, right);
});
...
export default function Inbox() {
  ...
  const [editingAction, setEditingAction] = useState<{ event: Event; action?: Action } | null>(null);
  ...
  const load = async (showLoading = true) => { if (showLoading) setLoading(true); try { const [eventList, actionList] = await Promise.all([eventsApi.list(), actionsApi.list()]); setEvents(eventList); setActions(actionList); setError(""); } catch (cause) { setError(userFacingError(cause)); } finally { if (showLoading) setLoading(false); } };
  const toggleEventExpanded = (eventId: number) => setExpandedEventIds((current) => { const next = new Set(current); if (next.has(eventId)) next.delete(eventId); else next.add(eventId); return next; });
  const reorderEventActions = async (eventId: number, actionIds: number[]) => {
    ...
    try { await actionsApi.reorderEvent({ event_id: eventId, action_ids: actionIds }); }
    catch (cause) { setActions(previous); setError(userFacingError(cause)); throw cause; }
  };
  useEffect(() => { void load(); }, []);
  useEffect(() => {
  ...
  const visibleEvents = useMemo(() => {
    const status: Record<InboxTab, number> = { pending: 0, projects: 1, delegated: 2, delayed: 3, abandoned: 4, completed: 5 };
    const filtered = events.filter((event) => event.status === status[filter]);
    if (filter !== "projects") return filtered;
    // 进行中事件按 P1、P2、P3、P4 排列；优先级数值越小越靠前。
    return sortByPriority(filtered.map((event) => ({
      ...event,
  ...
  const updateEvent = async (values: { title: string; target?: string; deadline?: Dayjs; importance?: number; urgency?: number }) => { if (!editing) return; try { await eventsApi.update({ id: editing.id, title: values.title.trim(), target: values.target?.trim() || undefined, deadline: toDateString(values.deadline), importance: Number(values.importance ?? 1), urgency: Number(values.urgency ?? 0) }); setEditing(null); await load(); message.success("事件已更新"); } catch (cause) { setError(userFacingError(cause)); } };
  const runProcess = async (payload: ProcessEvent) => { try { await eventsApi.process(payload); track("处理事件", { decision: payload.decision }); setProcessing(null); setFilter(payload.decision === "self" ? "projects" : payload.decision === "delegate" ? "delegated" : payload.decision === "delay" ? "delayed" : "abandoned"); await load(); message.success("事件处理完成"); } catch (cause) { setError(userFacingError(cause)); } };
  ...
  const complete = async (item: Event, closeProcessing = false) => { try { const result = await eventsApi.complete(item.id); if (closeProcessing) setProcessing(null); await load(); message.success(result && result.points_awarded > 0 ? `事件已完成，全部搞定 +${result.points_awarded} 积分` : "事件已完成"); } catch (cause) { setError(userFacingError(cause)); } };
  ...
  return <div className="page">{messageContextHolder}
    ...
    <div className={`record-list ${loading || visibleEvents.length === 0 ? "record-list-empty" : ""}`}>{loading ? <div className="card empty">正在加载…</div> : visibleEvents.length === 0 ? <div className={`card ${filter === "pending" ? "onboarding-empty" : "inbox-empty-state"}`}><Empty className="empty" image={Empty.PRESENTED_IMAGE_DEFAULT} description={filter === "pending" ? <div><Typography.Title level={4}>先把脑中的事情记下来</Typography.Title><Typography.Paragraph type="secondary">记录后再决定是自己做、委托、延后，还是放弃。</Typography.Paragraph><Button type="primary" onClick={() => document.querySelector<HTMLInputElement>(".quick-add input")?.focus()}>记录第一件事</Button></div> : "暂无事件"} /></div> : visibleEvents.map((item, index) => <EventRow key={item.id} item={item} index={index + 1} onEdit={setEditing} onProcess={(mode) => setProcessing({ event: item, mode })} onDelete={() => void remove(item)} onRestore={() => void restore(item)} onComplete={() => void complete(item)} expanded={expandedEventIds.has(item.id)} onToggle={() => toggleEventExpanded(item.id)} actions={actions.filter((action) => action.event_id === item.id || (item.project_id != null && action.project_id === item.project_id))} onAddAction={() => setEditingAction({ event: item })} onEditAction={(action) => setEditingAction({ event: item, action })} onDeleteAction={async (action) => { await actionsApi.delete(action.id); await load(false); message.success("行动已删除"); }} onReorder={(actionIds) => reorderEventActions(item.id, actionIds)} />)}</div>
    <Modal open={Boolean(editing)} title="编辑事件" onClose={() => setEditing(null)}>{editing && <Form className="form" layout="vertical" initialValues={{ title: editing.title, target: editing.target, deadline: editing.deadline ? dayjs(editing.deadline) : undefined, importance: editing.importance ?? 1, urgency: editing.urgency ?? 0 }} onFinish={(values) => void updateEvent(values)}><Form.Item label="事件标题" name="title" rules={[{ required: true, message: "请输入事件标题" }]}><Input autoFocus /></Form.Item><Form.Item label="事件目标" name="target"><Input.TextArea autoSize={{ minRows: 3, maxRows: 5 }} /></Form.Item><Form.Item label="截止日期" name="deadline"><DatePicker className="full-width" format="YYYY-MM-DD" /></Form.Item><div className="form-grid"><Form.Item name="importance" label="重要程度"><Select options={[{ value: 1, label: "重要" }, { value: 0, label: "不重要" }]} /></Form.Item><Form.Item name="urgency" label="紧急程度"><Select options={[{ value: 1, label: "紧急" }, { value: 0, label: "不紧急" }]} /></Form.Item></div><div className="form-footer"><Button onClick={() => setEditing(null)}>取消</Button><Button type="primary" htmlType="submit">保存</Button></div></Form>}</Modal>
    <EventActionModal data={editingAction} onClose={() => setEditingAction(null)} onSaved={async () => { setEditingAction(null); await load(false); }} />
    ...
    {guideNewEvent && <div className="inbox-guide-overlay" role="dialog" aria-label="添加事件引导">
      <div className="inbox-guide-callout">
        <Typography.Text>先在这里记录一个新事件，然后点击“去处理”，把它拆成一步步可直接执行的行动。</Typography.Text>
...
function EventRow({ item, index, onEdit, onProcess, onDelete, onRestore, onComplete, expanded, onToggle, actions, onAddAction, onEditAction, onDeleteAction, onReorder }: { item: Event; index: number; onEdit: (event: Event) => void; onProcess: (mode: ProcessMode) => void; onDelete: () => void; onRestore: () => void; onComplete: () => void; expanded: boolean; onToggle: () => void; actions: Action[]; onAddAction: () => void; onEditAction: (action: Action) => void; onDeleteAction: (action: Action) => Promise<void>; onReorder: (actionIds: number[]) => Promise<void> }) {
  const active = item.status === 0 || item.status === 3;
  const toggle = onToggle;
  if (item.status === 1) return <Card className="record-card project-card project-card-clickable" size="small" onClick={(event) => { if (event.target instanceof Element && event.target.closest("button, a, input, textarea, select, [role='button'], .project-actions-panel")) return; toggle(); }} title={<div className="project-card-header-shell"><div className="project-card-header"><Typography.Text strong className="project-card-title"><span className="card-index">{index}.</span>{item.title}</Typography.Text><Tag className="project-status-tag" color="blue">进行中</Tag><div className="project-card-meta"><button type="button" className="project-action-summary" aria-expanded={expanded} onClick={(event) => { event.stopPropagation(); toggle(); }}><ListChecks size={15} strokeWidth={1.8} aria-hidden="true" />行动 {item.completed_action_count}/{item.action_count}</button><span className="project-priority-group"><span className={`project-priority ${item.importance ? "project-priority-important" : "project-priority-muted"}`}>{item.importance ? "重要" : "不重要"}</span><span className={`project-priority ${item.urgency ? "project-priority-urgent" : "project-priority-muted"}`}>{item.urgency ? "紧急" : "不紧急"}</span></span></div></div><Space className="project-card-extra" onClick={(event) => event.stopPropagation()} size={4} wrap><Button size="small" type="text" icon={<Edit3 size={14} />} onClick={() => onEdit(item)}>编辑</Button>{item.completed_action_count === item.action_count && item.action_count > 0 && <Button size="small" type="primary" icon={<Check size={14} />} onClick={onComplete}>完成</Button>}<Button size="small" danger type="text" onClick={() => onProcess("abandon")}>放弃</Button><Popconfirm title="删除事件会同时删除全部行动，确定继续吗？" onConfirm={onDelete} okText="确定" cancelText="取消"><Button size="small" danger type="text" icon={<Trash2 size={14} />} /></Popconfirm></Space></div>}>{expanded && <EventActionList event={item} actions={actions} onAdd={onAddAction} onEdit={onEditAction} onDelete={onDeleteAction} onReorder={onReorder} />}</Card>;
  return <Card className="record-card event-card" size="small" title={<Typography.Text strong className="row-title"><span className="card-index">{index}.</span>{item.title}</Typography.Text>} extra={<Tag color={statusColors[item.status]}>{statusLabels[item.status]}</Tag>}><div className="event-card-body"><div className="row-meta"><span>{formatCreatedDate(item.created_at)}</span>{item.delegated_to && <span>委托给 {item.delegated_to}</span>}{item.delay_until && <span>推迟至 {item.delay_until}</span>}{item.action_count > 0 && <span>{item.action_count} 条行动 · {item.pending_action_count} 条待办</span>}</div><Space className="card-actions" size={4} wrap><Button size="small" type="text" icon={<Edit3 size={14} />} onClick={() => onEdit(item)}>编辑</Button>{item.status === 4 ? <Button size="small" type="text" icon={<RotateCcw size={14} />} onClick={onRestore}>恢复</Button> : active && <><Button size="small" type="primary" onClick={() => onProcess("self")}>去处理</Button><Button size="small" onClick={() => onProcess("delegate")}>委托</Button><Button size="small" onClick={() => onProcess("delay")}>推迟</Button><Button size="small" danger onClick={() => onProcess("abandon")}>放弃</Button></>}<Popconfirm title="删除事件会同时删除其全部行动，确定继续吗？" onConfirm={onDelete} okText="确定" cancelText="取消"><Button size="small" type="text" danger icon={<Trash2 size={15} />} /></Popconfirm></Space></div></Card>;
}

function EventActionList({ event, actions, onAdd, onEdit, onDelete, onReorder }: { event: Event; actions: Action[]; onAdd: () => void; onEdit: (action: Action) => void; onDelete: (action: Action) => Promise<void>; onReorder: (actionIds: number[]) => Promise<void> }) {
  type DropPosition = "before" | "after";
  const [orderedActions, setOrderedActions] = useState<Action[]>(() => sortEventActions(actions));
  ...
  const canReorder = orderedActions.length > 1;
  ...
  useEffect(() => { const next = sortEventActions(actions); orderedActionsRef.current = next; setOrderedActions(next); }, [actions]);
  ...
  const finishReorder = (targetId: number, position: DropPosition) => {
    ...
    if (sourceIndex < 0 || targetIndex < 0 || !canDropOn(previous[sourceIndex], previous[targetIndex])) { clearDropFeedback(); return; }
    const next = [...previous];
    ...
    void onReorder(next.map((action) => action.id))
      .then(() => message.success(`“${moved.title}”已移动到“${previous[targetIndex].title}”${position === "before" ? "之前" : "之后"}`))
      .catch(() => { orderedActionsRef.current = previous; setOrderedActions(previous); })
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (pointerEvent: PointerEvent) => {
      ...
      const element = document.elementFromPoint(pointerEvent.clientX, pointerEvent.clientY)?.closest<HTMLElement>("[data-event-action-id]");
      ...
      if (!source || !target || !canDropOn(source, target)) { dropTargetRef.current = null; setDragOverId(null); setDropPosition(null); return; }
      const bounds = element.getBoundingClientRect();
      const position: DropPosition = pointerEvent.clientY <= bounds.top + bounds.height / 2 ? "before" : "after";
  ...
  const startPointerDrag = (pointerEvent: ReactPointerEvent<HTMLElement>, actionId: number) => {
    if (!canReorderAction(orderedActions.find((action) => action.id === actionId) ?? actions[0]) || saving || pointerEvent.target instanceof Element && pointerEvent.target.closest("button, a, input, textarea, select")) return;
    pointerEvent.preventDefault();
  ...
  const renderAction = (action: Action) => {
    const actionIndex = orderedActions.findIndex((item) => item.id === action.id);
    const isDropTarget = dragOverId === action.id && dropPosition !== null;
    const dropClass = isDropTarget ? `is-drag-over-${dropPosition}` : "";
    return <List.Item data-event-action-id={action.id} className={`project-action-item ${canReorderAction(action) && !saving ? "is-reorderable" : ""} ${draggingId === action.id ? "is-dragging" : ""} ${dropClass}`} draggable={false} actions={[<Button key="edit" size="small" type="text" icon={<Edit3 size={13} />} onClick={() => onEdit(action)}>编辑</Button>, <Popconfirm key="delete" title="确定删除这个行动吗？" onConfirm={() => onDelete(action)} okText="确定" cancelText="取消"><Button size="small" danger type="text" icon={<Trash2 size={14} />} /></Popconfirm>]} onPointerDown={(pointerEvent) => startPointerDrag(pointerEvent, action.id)}><List.Item.Meta avatar={<span className={`project-action-drag-handle ${canReorderAction(action) && !saving ? "is-enabled" : ""}`} role="button" aria-label={canReorderAction(action) ? "拖动调整同日行动顺序" : "该日期暂无可调整的行动"} title={canReorderAction(action) ? "拖动调整同日顺序" : undefined}><GripVertical size={16} aria-hidden="true" /></span>} title={<span className={`project-action-title ${action.status === 1 ? "completed-title" : ""}`}><span className="project-action-number" aria-label={`第${actionIndex + 1}个行动`}>{actionIndex + 1}</span>{action.title}</span>} description={<Space className="action-meta" wrap><span className="action-start-meta">{action.start_date ? `${action.start_date} 开始` : "未设置开始日期"}</span><span className="action-duration-meta">{formatDuration(action.estimated_hours)}</span>{action.deadline && <span className="action-detail-meta">截止 {action.deadline}</span>}{action.description && <span className="action-detail-meta">{action.description}</span>}</Space>} />{isDropTarget && <span className="project-action-drop-label">放到此行动{dropPosition === "before" ? "之前" : "之后"}</span>}</List.Item>;
  };
  return <div className={`project-actions-panel ${saving ? "is-saving" : ""}`} onDragLeave={(dragEvent) => { if (dragEvent.currentTarget === dragEvent.target) clearDropFeedback(); }}>
    {event.target && <><div className="project-section-head"><Typography.Text strong>事件目标</Typography.Text></div><div className="project-target-panel"><Typography.Paragraph className="project-target-text">{event.target}</Typography.Paragraph></div></>}
    <div className="project-actions-head"><Space size={8}><Typography.Text strong>行动列表</Typography.Text>{canReorder && <Typography.Text type="secondary" className="project-action-sort-hint">{saving ? <span className="project-action-saving"><LoaderCircle size={13} aria-hidden="true" />正在保存</span> : draggingId ? "正在调整同日顺序" : "仅支持同日排序"}</Typography.Text>}</Space><Button size="small" type="primary" icon={<Plus size={14} />} onClick={onAdd}>添加行动</Button></div>
    {orderedActions.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="这个事件还没有行动" /> : <List dataSource={orderedActions} renderItem={renderAction} />}
  </div>;
}

function EventActionModal({ data, onClose, onSaved }: { data: { event: Event; action?: Action } | null; onClose: () => void; onSaved: () => Promise<void> }) {
  const [form] = Form.useForm(); useEffect(() => { form.setFieldsValue(data?.action ? { title: data.action.title, description: data.action.description, estimated_hours: data.action.estimated_hours || undefined, start_date: data.action.start_date ? dayjs(data.action.start_date) : undefined, deadline: data.action.deadline ? dayjs(data.action.deadline) : undefined, is_frog: data.action.is_frog === 1 } : { is_frog: false }); }, [data, form]); if (!data) return null;
  const submit = async (values: Record<string, unknown>) => { const payload = { title: values.title as string, description: values.description as string || undefined, estimated_hours: Number(values.estimated_hours || 0), start_date: toDateString(values.start_date as Dayjs | string | undefined), deadline: toDateString(values.deadline as Dayjs | string | undefined), is_frog: values.is_frog ? 1 : 0, importance: data.event.importance ?? 1, urgency: data.event.urgency ?? 0 }; try { if (data.action) await actionsApi.update({ ...payload, id: data.action.id } as UpdateAction); else await actionsApi.create({ ...payload, event_id: data.event.id }); await onSaved(); message.success(data.action ? "行动已更新" : "行动已添加"); } catch (cause) { message.error(userFacingError(cause)); } };
  return <Modal open title={data.action ? "编辑行动" : "添加行动"} onClose={onClose}><Form form={form} className="form" layout="vertical" onFinish={(values) => void submit(values)}><Form.Item className="full" name="title" label="行动标题" rules={[{ required: true, message: "请输入行动标题" }]}><Input autoFocus /></Form.Item><div className="form-grid action-modal-grid"><Form.Item name="estimated_hours" label="预计耗时（选填）"><Select options={hours} /></Form.Item><Form.Item name="start_date" label="开始日期（选填）"><DatePicker className="full-width" format="YYYY-MM-DD" /></Form.Item><Form.Item name="deadline" label="截止日期"><DatePicker className="full-width" format="YYYY-MM-DD" /></Form.Item></div><Form.Item name="description" label="描述"><Input.TextArea autoSize={{ minRows: 2, maxRows: 4 }} /></Form.Item><Form.Item name="is_frog" valuePropName="checked"><Checkbox>标记为青蛙</Checkbox></Form.Item><div className="form-footer"><Button onClick={onClose}>取消</Button><Button type="primary" htmlType="submit">保存</Button></div></Form></Modal>;
}
