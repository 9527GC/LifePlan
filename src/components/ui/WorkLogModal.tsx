...
export default function WorkLogModal({ date, slots, onClose }: { date: string; slots: DailyScheduleSlot[]; onClose: () => void }) {
  ...
  return <>
    ...
    <Modal className="ai-settings-modal" wrapClassName="ai-settings-modal-wrap" open={settingsOpen} title="AI 配置" style={{ top: 24, marginBottom: 24 }} onCancel={() => setSettingsOpen(false)} footer={<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><Button type="link" onClick={resetAiDefaults}>恢复默认配置（不清除 API Key）</Button><Space><Button onClick={() => setSettingsOpen(false)}>取消</Button><Button type="primary" onClick={saveSettings}>保存配置</Button></Space></div>} destroyOnHidden>
