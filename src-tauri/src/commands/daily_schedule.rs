...
fn slot_query(conn: &Connection, list_date: &str) -> rusqlite::Result<Vec<DailyScheduleSlot>> {
    ...
    let mut statement = conn.prepare("SELECT s.id, s.list_date, s.start_time, s.end_time, s.action_id, s.actual_notes, s.met_expectation, s.focused, s.sort_order, a.id, COALESCE(a.event_id, p.event_id), a.project_id, COALESCE(e.title, pe.title), p.title, COALESCE(e.delegated_to, pe.delegated_to), a.title, a.description, a.estimated_hours, a.start_date, a.deadline, a.is_frog, COALESCE(p.importance, a.importance), COALESCE(p.urgency, a.urgency), COALESCE(p.priority, a.priority), a.status, a.completed_at, a.is_delegated_follow_up, a.cascade_abandoned, COALESCE(a.sort_order, 0), a.created_at, a.updated_at FROM daily_schedule_slots s LEFT JOIN actions a ON a.id = s.action_id AND a.space_id = s.space_id AND a.deleted_at IS NULL LEFT JOIN events e ON e.id = a.event_id AND e.space_id = a.space_id AND e.deleted_at IS NULL LEFT JOIN projects p ON p.id = a.project_id AND p.space_id = a.space_id AND p.deleted_at IS NULL LEFT JOIN events pe ON pe.id = p.event_id AND pe.space_id = p.space_id AND pe.deleted_at IS NULL WHERE s.space_id = ?1 AND s.list_date = ?2 ORDER BY s.start_time, s.id")?;
